...
export const extensionController = {
    ...
    syncSamples: async (req: Request, res: Response) => {
        try {
            ...
            if (!samples || !Array.isArray(samples)) {
                return res.status(400).json({ success: false, message: "Invalid data format. 'samples' array required." });
            }
    ...
    getExistingIds: async (req: Request, res: Response) => {
        try {
            ...
            const existing = await SampleRequest.find({ userId: user.id }).select('requestId').lean();
